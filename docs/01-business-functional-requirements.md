...
| FR-4.5 | Учёт стоимости               | Система должна вести учёт стоимости обработки запроса (токены × стоимость GPU-часа) в разрезе роли и сценария для отчёта FinOps.                                                                                              | Should (Важно)     |
...
| B — арендованный GPU     | Нагрузочный тест и отчёт (2–4 часа аренды)             | 1 × A100 40 ГБ или аналог; модель 14–30B в AWQ, vLLM с KV-cache  | TTFT p95 ≤ 1,5 с; ≥ 40 токенов/с на поток; ≥ 5 RPS при 20 одновременных сессиях |
