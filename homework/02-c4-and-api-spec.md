...
грифов. Сервис агента роль не интерпретирует и правами не управляет: он получает
готовый перечень и подставляет его в запросы к хранилищам.

Это разделение делает права проверяемыми в отрыве от аутентификации: тест на
...
AnswerRequest:
  type: object
  required: [question, session_id, role, acl]
  properties:
    question: { type: string, minLength: 3, maxLength: 2000 }
    session_id: { type: string, format: uuid }
    role: { type: string, enum: [buyer, manager, procurer, content, admin] }
    acl:
      type: array
      minItems: 1
